      ...
      {rooms.map(r => (
        <div key={r.id} className="flex items-center justify-between px-3 py-2.5 bg-zinc-900 border border-zinc-700 rounded-lg">
          <div>
            ...
            <span className="ml-2 text-sm text-zinc-400 font-mono tracking-widest">{r.id}</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-sm text-emerald-500">● Waiting</span>
            <button
              onClick={() => onJoin(r.id)}
              className="min-h-11 px-3 py-2 bg-sky-700 hover:bg-sky-600 text-zinc-100 rounded text-sm font-medium transition-colors"
            >
              Join
            </button>
          </div>
