  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (const p of particles) {
        ...
        if (p.y > activeCanvas.height + 10) { p.y = -10; p.x = Math.random() * activeCanvas.width }
        activeContext.save()
        activeContext.globalAlpha = 0.88
        activeContext.translate(p.x, p.y)
        activeContext.rotate(p.rot)
        activeContext.fillStyle = p.color
        activeContext.fillRect(-p.w / 2, -p.h / 2, p.w, p.h)
...
export default function GameOverOverlay({ result, onDismiss }: GameOverOverlayProps) {
