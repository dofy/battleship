...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      {/* Main content */}
      <main className="mx-auto flex max-w-3xl flex-col gap-5 px-4 py-5 sm:flex-row sm:gap-7 sm:px-6 sm:py-8">

        {/* Controls */}
        <div className="w-full space-y-3 sm:w-60 sm:flex-shrink-0">
          {/* Persistent player identity */}
          <Card className="border-zinc-700" aria-labelledby="player-identity-heading">
          ...
          {/* Choose opponent and create battle */}
          <section className="space-y-2.5" aria-labelledby="create-battle-heading">
            <div id="create-battle-heading" className="text-xs font-bold uppercase tracking-widest text-zinc-400">
              Choose opponent
            </div>
            ...
            {battleMode === 'online' && (
              <label htmlFor="public-room" className="flex min-h-11 cursor-pointer select-none items-center gap-3 rounded-lg px-1 text-sm text-zinc-300">
                <Switch
                  id="public-room"
                  checked={isPublic}
                ...
                <span className="min-w-0">
                  ...
                  <span className="block text-xs text-zinc-500">Listed in the open-battle lobby</span>
                </span>
              </label>
            )}
            <Button
              ...
              className="w-full tracking-widest disabled:cursor-wait"
            >
              {battleMode === 'computer'
                ? <Bot className="size-4" aria-hidden="true" />
          ...
          {/* Join by code */}
          <section className="space-y-2 border-t border-zinc-800 pt-3" aria-labelledby="join-battle-heading">
            <div id="join-battle-heading" className="text-xs font-bold uppercase tracking-widest text-zinc-400">
              Join a battle
            </div>
            <div className="flex gap-2">
              <Input
                ...
                className="min-w-0 flex-1 bg-zinc-800 font-mono tracking-widest"
                autoComplete="off"
              ...
              <Button
                ...
                onClick={() => handleJoin()}
                disabled={!!pendingAction}
                className="border-sky-800 text-sky-300 hover:border-sky-600 hover:bg-sky-950"
              >
          ...
          {/* Combat record */}
          <Card className="rounded-lg border-zinc-700">
            <CardContent className="p-3">
              <div className="mb-2 text-xs font-bold uppercase tracking-widest text-sky-400">Combat Record</div>
